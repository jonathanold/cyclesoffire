...
forv b = 1/`n_fes' { 
	...
	forv t = 1/`n_types' {
		...
		forv a = 1/`n_outcomes' { 
			...
			if "`fe'" == "year" {
				estadd local yfe "Year"
				}
			else if "`fe'" == "year_province" {
				estadd local yfe "Prov-year"
				}
			est sto `type'_`outcome'_`fename'																			

		}

	/* [> Make combined tables <] */ 
	#delimit ;
	estout `type'_forest_`fename' `type'_concession_`fename'  `type'_palm_`fename' `type'_fibre_`fename' `type'_logging_`fename'  `type'_nomansland_`fename' `type'_protected_`fename'
	using   "${output}/tables/table_b`tablenumber'.tex", style(tex)  
	wrap varwidth(25) 
	varlabels(
	...
	stats(obs1 omean1 dfe yfe test1 diff2 test2, 
		labels("\midrule \addlinespace Observations"  "Mean of DV" "Spatial FE" "Temporal FE" "Joint p-value"  "\textbf{This vs. last:} \\ \hspace{0.2cm} Difference" "  \hspace{0.2cm} p-value"))
	stardrop(`spec')
...
foreach exclude_var of varlist is_child is_parent { 
	...
	;
	graph export "${output}/figures/figure_c`figurenumber'.pdf", as(pdf) replace
	;
	#delimit cr 
}
